    ...
    <section className="grid grid-cols-1 gap-4">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
        <AppCard className="lg:col-span-6">
          ...
          <button
            onClick={() => {
              ...
              setBills((prev) => [...prev, { id: crypto.randomUUID(), name: billName.trim(), amount: Number(billAmount), dueDate: new Date().toISOString().slice(0,10), paid: false }]);
            ...
            {bills.length === 0 ? <p className="rounded-lg border border-dashed border-white/20 p-4 text-sm text-slate-400">No bills yet.</p> : bills.map((bill) => (
              <div key={bill.id} className="rounded-lg border border-white/10 bg-[#0a1020]/70 p-2">
                <div className="flex items-center gap-2">
                  ...
                  <input value={bill.amount} type="number" onChange={(e) => setBills((prev) => prev.map((x) => x.id === bill.id ? { ...x, amount: Number(e.target.value) } : x))} className="w-24 rounded border border-white/10 bg-[#0d1629] px-2 py-1 text-xs" />
        ...
        <AppCard className="lg:col-span-6">
          ...
          <button onClick={() => { if (!subName.trim() || !subAmount) return; setSubs((prev) => [...prev, { id: crypto.randomUUID(), name: subName.trim(), amount: Number(subAmount), cycle: "monthly" }]); setSubName(""); setSubAmount(""); }} className="mb-3 h-10 w-full rounded-lg bg-blue-500/80 text-sm">Add Subscription</button>
          <div className="space-y-2">
            {subs.length === 0 ? <p className="rounded-lg border border-dashed border-white/20 p-4 text-sm text-slate-400">No subscriptions yet.</p> : subs.map((s) => (
              <div key={s.id} className="flex items-center gap-2 rounded-lg border border-white/10 bg-[#0a1020]/70 p-2">
                ...
                <input value={s.amount} type="number" onChange={(e) => setSubs((prev) => prev.map((x) => x.id === s.id ? { ...x, amount: Number(e.target.value) } : x))} className="w-20 rounded border border-white/10 bg-[#0d1629] px-2 py-1 text-xs" />
      ...
      <AppCard>
        ...
        <button onClick={() => { if (!budgetName.trim() || !budgetLimit) return; setBudgets((prev) => [...prev, { id: crypto.randomUUID(), name: budgetName.trim(), limit: Number(budgetLimit), spent: 0 }]); setBudgetName(""); setBudgetLimit(""); }} className="mb-3 h-10 w-full rounded-lg bg-blue-500/80 text-sm">Add Category</button>
        <div className="space-y-2">
          {budgets.length === 0 ? <p className="rounded-lg border border-dashed border-white/20 p-4 text-sm text-slate-400">No categories yet.</p> : budgets.map((b) => (
            <div key={b.id} className="rounded-lg border border-white/10 bg-[#0a1020]/70 p-2">
              ...
              <div className="mt-2 flex gap-2">
                <input type="number" value={b.spent} onChange={(e) => setBudgets((prev) => prev.map((x) => x.id === b.id ? { ...x, spent: Number(e.target.value) } : x))} className="h-8 w-full rounded border border-white/10 bg-[#0d1629] px-2 text-xs" />
